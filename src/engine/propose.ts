...
function suggestionCount(fatigue: Fatigue): number {
  switch (fatigue) {
    case '限界':
    case '疲れた':
      return 1;
...
export async function getSuggestions(opts: SuggestOpts): Promise<SuggestManyOutput> {
  ...
  // 1. オンデバイス
  if (onDeviceEnabled) {
    const od = await collectFromSource(
      async () => {
        if (!(await isOnDeviceUsable())) throw new Error('on-device unusable');
        return proposeOnDevice(buildDecisionInput(opts, profile), fridgeNames(opts), opts.members.length);
      },
      opts, profile, avoid, count, 'od',
    );
